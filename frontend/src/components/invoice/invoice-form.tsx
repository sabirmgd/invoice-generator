...
export function InvoiceForm({ onInvoiceCreated }: InvoiceFormProps) {
  ...
  async function handleSubmit(e: FormEvent, asDraft: boolean) {
    ...
    try {
      ...
      const created = await apiFetch<Invoice>('/api/v1/invoices', {
        method: 'POST',
        body: JSON.stringify(payload),
      });

      // If not a draft, mark as sent
      if (!asDraft) {
        await apiFetch(`/api/v1/invoices/${created.id}/status`, {
          method: 'PATCH',
      ...
      // Use refreshed invoice data to get the updated status
      const refreshedInvoice = data?.invoices?.find((inv) => inv.id === created.id) || created;
      onInvoiceCreated(refreshedInvoice);
    ...
    <div className="flex h-full flex-col overflow-y-auto">
      <form className="flex flex-col gap-5 p-5" onSubmit={(e) => handleSubmit(e, false)}>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          {/* Bill To */}
          <div className="flex flex-col gap-1.5">
            <span className="text-xs font-medium text-text-secondary uppercase tracking-wide">Bill To</span>
